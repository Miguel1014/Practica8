...
namespace Practica8
{
    ...
    public partial class MainPage : ContentPage
    {
        ...
        private async void FullDayLeave_Clicked(object sender, EventArgs e)
        {
            ...
            Client = new GraphServiceClient("https://graph.microsoft.com/v1.0/me/sendMail",
                      new DelegateAuthenticationProvider(
                          async (requestMessage) =>
                          {
                              var tokenRequest = await App.IdentityClientApp.AcquireTokenAsync(App.Scopes, App.UiParent).ConfigureAwait(false);
          ...
          await  GetMyDetailsAsync();
            ...
            {
                ToRecipients = new List<Recipient>() { new Recipient() { EmailAddress = new EmailAddress() { Address = ((User)Me).UserPrincipalName } } },
